...
for ((i=0; i<${3:-3}; i++))
do
  multipass launch 22.04 -n vmk"$i" -c 2 -m 2G -d 10G --cloud-init - <<EOF
  # cloud-config
  runcmd:
...
for ((ip_id=0; ip_id<${#ip_addresses[@]}; ip_id++)); do
    ...
    if [ $ip_id -eq 0 ]; then
	k3sup install --ip=$master_ip --merge --user=root --sudo --cluster --k3s-version=v1.27.3+k3s1 --k3s-extra-args "--flannel-backend=none --cluster-cidr=10.244.0.0/16 --disable-network-policy --disable-kube-proxy --disable traefik --disable servicelb --node-ip=$master_ip" --local-path $HOME/.kube/config --context=kpr
    else
