...
for ((i=0; i<${1:-1}; i++))
do
  multipass launch 20.04 -n vm"$i" -c $cpu -m $mem -d 30G --cloud-init - <<EOF
  # cloud-config
  runcmd:
    ...
    - sudo wget -r -np -nH --cut-dirs=2 --directory-prefix=/root/ http://192.168.2.100/k3s/vmenv/kindenv/ && rm -rf /root/kindenv/index.html
EOF
done

mapfile -t ip_addresses < <(multipass list | grep vm[0-9] | awk '{print $3}')
